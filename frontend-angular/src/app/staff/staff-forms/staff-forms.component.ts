...
import { StaffForm } from './staff-forms.component.model';
...
@Component({
  selector: "app-staff-forms",
  templateUrl: "./staff-forms.component.html",
  styleUrls: ["./staff-forms.component.css"],
  providers: [StaffService, ConstantService, AuthHelper],
})
export class StaffFormsComponent {
  ...
  constructor(
    public staffFormsService: StaffService,
  ...
  load() {
    this.myLoader.open();
    this.staffFormsService.GetAllStaffForms().subscribe(
      (data: any) => {

        if (data.result) {
          this.staffForms = data.staffForms;
        } else {
