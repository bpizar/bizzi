...
import { ClientForm } from './client-forms.component.model';
...
export class ClientFormsComponent {
  ...
  constructor(
    public clientFormsService: ClientsService,
  ...
  load() {
    this.myLoader.open();
    this.clientFormsService.GetAllClientForms().subscribe(
      (data: any) => {

        if (data.result) {
          this.clientForms = data.clientForms;
        } else {
