...
@Component({
    selector: 'reportschedulebystaff',
    templateUrl: 'reportschedulebystaff.component.template.html',
    providers: [ConstantService, StaffService, SchedulingService, AuthHelper, CommonHelper],
    styleUrls: ['../styleReport.component.css'],
})

@Injectable()
export class ReportScheduleByStaff implements OnInit, AfterViewInit {
    ...
    titleReport: string = "Schedule by Staff";
    @Input('layout') layout: string = "";
    columns: string[] = []; // [ "From", "To","hours"]; // "projectName","assignedToPosition",
    groupsLabels: string[] = []; // ["Project", "Position",];
    groupsColumns: string[] = ["projectName", "assignedToPosition"];
    @Input('columnsWidth') columnsWidth: string[] = ["300", "300", "300"];
    @ViewChild('glowmessage') glowMessage: GlowMessages;
    @ViewChild('staffsDropDown') staffsDropDown: jqxListBoxComponent;
    ...
    dataAdapterStaff: any = new jqx.dataAdapter(this.sourceStaffsListBox);

    staffsBindingComplete = (event: any) => {
        if (this.sourceStaffsListBox.localData != undefined) {
            if (this.sourceStaffsListBox.localData.length > 0) {
                this.staffsDropDown.selectedIndex(0);
            }
        }
    }

    StaffsSelectDropDown = (event: any): void => {
        this.currentStaff = event.args.item.value;
    ...
    LoadReport = (): void => {
        if (this.currentStaff != 0) {
            this.myLoader.open();
            this.schedulingService.GetSchedulingByStaff(this.currentStaff/*,this.currentPeriod*/)
                .subscribe(
                    (rdata: any) => {
                        if (rdata.result) {
                            let data: RowModel[] = [];

                            if (rdata.scheduling.length > 0) {
                                this.subtitleReport = this.subtitleReportAux + " : " + rdata.scheduling[0].assignedToFullName;
                                this.subtitleReport2 = this.subtitleReport2Aux + " : " + this.periodsDropDown.getSelectedItem().label;
                            }

                            for (var i = 0; i < rdata.scheduling.length; i++) {

                                let newrow = new RowModel();
                                let newcell1 = new CellModel("projectName", 10);
                                newcell1.value = rdata.scheduling[i].projectName;
                                newrow.cells.push(newcell1);

                                let newcell43 = new CellModel("assignedToPosition", 40);
                                newcell43.value = rdata.scheduling[i].assignedToPosition;
                                newrow.cells.push(newcell43);

                                let newcell5 = new CellModel("from", 10);
                                newcell5.value = rdata.scheduling[i].from.replace("T", " ");
                                newrow.cells.push(newcell5);

                                let newcell3 = new CellModel("to", 40);
                                newcell3.value = rdata.scheduling[i].to.replace("T", " ");
                                newrow.cells.push(newcell3);

                                let newcell7 = new CellModel("hours", 40);
                                newcell7.value = this.commonHelper.convertMinsToHrsMins(rdata.scheduling[i].hours);
                                newrow.cells.push(newcell7);
    ...
    ngAfterViewInit(): void {
        setTimeout(() => {
            ...
            this.translate.get('report_name_2_period').subscribe((resPeriod: string) => {
                this.translate.get('report_name_2_usuario').subscribe((resUsuario: string) => {
                    this.translate.get('report_name_2').subscribe((res: string) => {
                        this.titleReport = res;
                        this.subtitleReportAux = resUsuario;
                        this.subtitleReport2Aux = resPeriod;

                        this.translate.get('report_name_2_col_0').subscribe((resCol0: string) => {
                            this.translate.get('report_name_2_col_1').subscribe((resCol1: string) => {
                                this.translate.get('report_name_2_col_2').subscribe((resCol2: string) => {
                                    this.translate.get('report_name_2_grouplabel_0').subscribe((resGroup0: string) => {
                                        this.translate.get('report_name_2_grouplabel_1').subscribe((resGroup1: string) => {
                                            ...
                                            this.staffService.GetAllStaff()
                                                .subscribe(
                                                    (data: any) => {
                                                        if (data.result) {
                                                            this.sourceStaffsListBox.localData = data.staffs;
                                                            this.dataAdapterStaff = new jqx.dataAdapter(this.sourceStaffsListBox);
                                                        }
                                                        else {
