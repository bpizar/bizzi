...
@Component({
    selector: 'reportschedulebyproject',
    templateUrl: 'reportschedulebyproject.component.template.html',
    providers: [ConstantService, ProjectsService, SchedulingService, AuthHelper, CommonHelper],
    styleUrls: ['../styleReport.component.css'],
})

@Injectable()
export class ReportScheduleByProject implements OnInit, AfterViewInit {
    ...
    titleReport: string = "";
    columns: string[] = [];
    groupsColumns: string[] = ["assignedToFullName", "assignedToPosition"];;
    groupsLabels: string[] = [];
    columnsWidth: string[] = ["300", "300", "300"];
    ...
    LoadReport = (): void => {
        if (this.currentPeriod != 0 && this.currentProject != 0) {
            ...
            this.schedulingService.GetSchedulingByProject(this.currentProject, this.currentPeriod)
                .subscribe(
                    (rdata: any) => {
                        if (rdata.result) {
                            let data: RowModel[] = [];
                            ...
                            if (rdata.scheduling.length > 0) {
                                this.subtitleReport = this.subtitleReportAux + " : " + rdata.scheduling[0].projectName;
                            ...
                            for (var i = 0; i < rdata.scheduling.length; i++) {
                                ...
                                let newcell4 = new CellModel("assignedToFullName", 40);
                                newcell4.value = rdata.scheduling[i].assignedToFullName;
                                newrow.cells.push(newcell4);

                                let newcell65 = new CellModel("assignedToPosition", 40);
                                newcell65.value = rdata.scheduling[i].assignedToPosition;
                                ...
                                let newcell334 = new CellModel("hours", 40);
                                newcell334.value = this.commonHelper.convertMinsToHrsMins(rdata.scheduling[i].hours);
    ...
    ngAfterViewInit(): void {
        setTimeout(() => {
            ...
            this.translate.get('report_name_1_period').subscribe((resPeriod: string) => {
                this.translate.get('report_name_1_project').subscribe((resProjecto: string) => {
                    this.translate.get('report_name_1').subscribe((res: string) => {
                        this.titleReport = res;
                        this.subtitleReportAux = resProjecto;
                        this.subtitleReport2Aux = resPeriod;

                        this.translate.get('report_name_1_col_0').subscribe((resCol0: string) => {
                            this.translate.get('report_name_1_col_1').subscribe((resCol1: string) => {
                                this.translate.get('report_name_1_col_2').subscribe((resCol2: string) => {
                                    this.translate.get('report_name_1_grouplabel_0').subscribe((resGroup0: string) => {
                                        this.translate.get('report_name_1_grouplabel_1').subscribe((resGroup1: string) => {
